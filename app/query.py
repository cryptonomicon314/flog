...
def all_visible_entries(db, now, start, page_size, is_preview, archivable=True):
    if is_preview:
    ...
    if (start, page_size) != (None, None):
        q = q.offset(start).limit(page_size)
    return q

def visible_entries(db, catslug, now, start, page_size, is_preview, archivable=True):
    if is_preview:
        q = db.session.query(Entry)\
